...
myeconomist_pal <- function(fill=TRUE) {
  colors <- deframe(ggthemes::ggthemes_data[["myeconomist"]][["fg"]])
  if (fill) {
    max_n <- 9
    f <- function(n) {
      ...
      if (n == 1L) {
        ...
        i <- c("light blue", "lgiht green")
      } else if (n == 3L) {
        i <- c("light blue", "light green", "dark green")
      } else if (n == 4L) {
        i <- c("blue", "light blue", "light green", "dark green")
      } else if (n == 5L) {
        ## 20120901_woc904
        i <- c("blue", "light blue", "blue-gray", "ligt green",
               "dark green")
      } else if (n == 6L) {
        ## 20120901_woc904
        i <- c("blue", "light blue", "blue-gray", "gray",
...
#' @export
scale_colour_myeconomist <- function(...) {
  discrete_scale("colour", "economist", myeconomist_pal(), ...)
}

#' @rdname scale_economist
#' @export
scale_color_myeconomist <- scale_colour_myeconomist

#' @rdname scale_economist
#' @export
scale_fill_myeconomist <- function(...) {
  discrete_scale("fill", "myeconomist", myeconomist_pal(), ...)
}
...
theme_myeconomist <- function(base_size = 10, base_family = "sans",
                            horizontal = TRUE, dkpanel = FALSE) {
  bgcolors <- deframe(ggthemes::ggthemes_data[["myeconomist"]][["bg"]])
  ## From measurements
...
#' @export
theme_myeconomist_white <- function(base_size = 11, base_family = "sans",
                                  gray_bg = TRUE, horizontal = TRUE) {
  if (gray_bg) {
    bgcolor <- get_colors(c("myeconomist", "bg"), "light gray")
  } else {
    bgcolor <- "white"
  }
  theme_myeconomist(base_family = base_family,
                  base_size = base_size,
